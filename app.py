...
# ════════════════════════════════════════════════════════════════
    ...
    if supply_inventory:
        ...
        st.dataframe(
            df_sup,
            column_config={"image_url": st.column_config.ImageColumn("画像"), "name": "資材名", "category": "カテゴリ", "current_stock": st.column_config.NumberColumn("現在庫", format="%d")},
...
# ════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="main-header"><h1>🔍 双方向原料トレース (HACCP対応)</h1><p>原料の行方（フォワード）と製品の構成（バックワード）を双方向で追跡</p></div>', unsafe_allow_html=True)

    tab_fwd, tab_bwd = st.tabs(["➡️ 原料から製品を追跡 (フォワード)", "⬅️ 製品から原料を遡る (バックワード)"])

    with tab_fwd:
        st.info("特定の原料が **いつ・どの製品に・どれだけ使われたか** を特定します（不良原料の回収・影響範囲特定用）。")
        st.markdown('<div class="form-card">', unsafe_allow_html=True)
        kw_fwd = st.text_input("検索する原料の「ロットNo」または「入荷No」", placeholder="例: 1-109", key="kw_fwd")
        st.markdown('</div>', unsafe_allow_html=True)
        
        if kw_fwd and st.button("➡️ 追跡実行", type="primary", key="btn_fwd"):
            kw_l = kw_fwd.lower()
            arr_info = [a for a in arrivals if kw_l in str(a.get("lot_no","")).lower() or kw_l in str(a.get("arrival_no","")).lower()]
            
            if arr_info:
                st.markdown("#### 📦 対象の原料入荷記録")
                st.dataframe(pd.DataFrame(arr_info)[["arrival_no","arrival_date","maker","material_type","lot_no","bags","appearance"]], use_container_width=True, hide_index=True)
            ...
            for b in brewing:
                ...
                if is_match:
                    res_fwd.append({
                        "仕込No": b.get("no",""), "仕込日": b.get("brew_date",""), 
                        "品名": b.get("product_name",""), "仕込量(kg)": b.get("brew_amount",0),
                        "主ロット": b.get("lot_no",""), "海藻ロット": b.get("seaweed_lot",""), "デンプンロット": b.get("starch_lot","")
                    })
            if res_fwd:
                st.success(f"✅ この原料が使用された製品が **{len(res_fwd)}件** 見つかりました。")
                st.dataframe(pd.DataFrame(res_fwd), use_container_width=True, hide_index=True)
            else:
                st.warning("この原料はまだ製品に使用されていません。")

    with tab_bwd:
        st.info("特定の製品に使われた **すべての原料とその入荷元・メーカー・検査結果** を特定します（クレーム原因調査用）。")
        st.markdown('<div class="form-card">', unsafe_allow_html=True)
        ...
        if st.button("⬅️ 遡及実行", type="primary", key="btn_bwd"):
            ...
            if not target_brews:
                ...
                st.success(f"✅ **{len(target_brews)}件** の仕込み記録が見つかりました。")
                for tb in target_brews:
                    st.markdown(f"### 🧪 仕込No: {tb.get('no')} - {tb.get('brew_date')} 【{tb.get('product_name')}】")
                    used_lots = []
                    for k, n in [("lot_no","主原料(精粉)"), ("seaweed_lot","海藻粉"), ("starch_lot","加工デンプン")]:
                        if tb.get(k) and tb.get(k) != "─": used_lots.append({"原料種別": n, "ロットNo": tb.get(k)})
                    if tb.get("other_additives"):
                        try:
                            for o in json.loads(tb["other_additives"]):
                                if o.get("lot") and o.get("lot") != "─": used_lots.append({"原料種別": o.get("name","添加物"), "ロットNo": o.get("lot")})
                        except: pass
                    
                    res_bwd = []
                    for ul in used_lots:
                        arr_match = next((a for a in arrivals if str(a.get("lot_no","")) == str(ul["ロットNo"])), None)
                        if arr_match:
                            res_bwd.append({
                                "役割": ul["原料種別"], "ロットNo": ul["ロットNo"], "メーカー": arr_match.get("maker",""), 
                                "入荷日": arr_match.get("arrival_date",""), "入荷時外観": arr_match.get("appearance","")
                            })
                        else:
                            res_bwd.append({"役割": ul["原料種別"], "ロットNo": ul["ロットNo"], "メーカー": "不明", "入荷日": "-", "入荷時外観": "-"})
                    st.dataframe(pd.DataFrame(res_bwd), use_container_width=True, hide_index=True)
                    st.markdown("---")
