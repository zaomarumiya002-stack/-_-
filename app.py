...
st.markdown("""
<style>
:root{ --c-bg:#f1f5f9; --c-surface:#ffffff; --c-primary:#163b66; --c-primary-light:#2f6fb0; --c-border:#94a3b8; }
...
.stTextInput input, .stNumberInput input, .stSelectbox div[data-baseweb="select"], .stTextArea textarea, .stDateInput input {
    background-color: #ffffff !important; border: 2px solid var(--c-border) !important;
    border-radius: 8px !important; color: #0f172a !important; font-size: 1rem !important; padding: 12px !important;
}
.stTextInput input:focus, .stNumberInput input:focus, .stSelectbox div[data-baseweb="select"]:focus-within {
    border-color: var(--c-primary) !important; box-shadow: 0 0 0 3px rgba(22, 59, 102, 0.1) !important;
}
label { color: #1e293b !important; font-weight: 700 !important; font-size: 0.95rem !important; }
.main-header{ background:linear-gradient(120deg, #163b66, #2f6fb0); padding:24px; border-radius:12px; margin-bottom:24px; color:white; }
.form-card{ background:var(--c-surface); border:1px solid #cbd5e1; border-radius:12px; padding:24px; margin-bottom:20px; box-shadow:0 4px 6px -1px rgba(0,0,0,0.1); }
.section-title{ font-size:1.1rem; font-weight:800; color:var(--c-primary); margin-bottom:15px; border-bottom:2px solid #e3eefb; padding-bottom:5px; }
</style>
...
if page == "🏠 ダッシュボード":
    ...
    t_calc, t_list = st.tabs(["➕ 配合計算・新規登録", "📋 記録一覧・修正"])
    with t_calc:
        st.markdown('<div class="form-card">', unsafe_allow_html=True)
        p_name = st.selectbox("品名を選択", list(st.session_state.recipe_master.keys()))
        target_kg = st.number_input("希望仕込み量 (kg)", min_value=0, value=100, step=10)
        st.subheader("⚙️ 配合内訳 (kg)")
        recipe = st.session_state.recipe_master[p_name]
        cols = st.columns(4)
        calc_values = {k: cols[i%4].number_input(f"{k} (kg)", value=int(target_kg * (v / 100)), step=1) for i, (k, v) in enumerate(recipe.items())}
        st.markdown('</div>', unsafe_allow_html=True)
        if st.button("✅ 仕込み記録に登録", type="primary", use_container_width=True):
            append_brewing({
                "仕込No": next_brewing_no(brewing), "仕込日": str(date.today()), "品名": p_name,
                "仕込量(kg)": target_kg, "こんにゃく精粉(kg)": calc_values.get("原料", 0),
                "その他添加物": json.dumps(calc_values, ensure_ascii=False), "登録日時": datetime.now().isoformat()
            })
