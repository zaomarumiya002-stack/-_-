...
# ════════════════════════════════════════════════════════════════
#  原料 使用履歴(製造仕込み記録の「その他添加物」明細から原料別・日付別に集計)
#  経営ダッシュボードの「使用履歴」表示で使用する。
# ════════════════════════════════════════════════════════════════
def get_material_usage_history():
    usage_by_material = {}   # 原料名 -> [{"日付":..., "kg":...}, ...]
    usage_events = []        # 個別使用イベント一覧(直近履歴の詳細表示用)
    for b in brewing:
        oa = b.get("その他添加物", "")
        if not oa: continue
        ...
        for item in items:
            ...
            try: kg = float(item.get("kg", 0.0) or 0.0)
            except Exception: kg = 0.0
...
# ════════════════════════════════════════════════════════════════
if page == "📊 経営ダッシュボード":
    st.markdown('<div class="main-header"><h1>📦 主要原料在庫モニター</h1><p>主要原料の在庫状況と使用履歴を、このタブだけでひと目で把握できます。</p></div>', unsafe_allow_html=True)
    ...
    for idx, m in enumerate(materials):
        ...
        # 直近7日間の使用量(その他添加物の明細から原料名で集計)を各カードに小さく表示
        recent7_kg = sum(e["kg"] for e in material_usage_by_name.get(m, []) if _days_ago(e["日付"]) <= 7)
        with cols[idx % 4]:
            with st.container(border=True):
                ...
                st.caption(f"📉 直近7日間の使用量: {fmt_kg(recent7_kg)} kg")
                if is_alert:
                    st.markdown('<div class="status-badge danger" style="margin-top:10px;">⚠ 在庫不足</div>', unsafe_allow_html=True)

    # ════════════════════════════════════════════════════════════
    #  ★【新規実装】使用履歴を同一タブ内で視覚的に確認できる機能
    #    原料を選ぶと、日別の使用量推移グラフと詳細な使用履歴一覧が
    #    その場に表示される。製造仕込みタブで記録した「その他添加物」の
    #    明細(原料名・使用量・日付)を集計して作成している。
    # ════════════════════════════════════════════════════════════
    st.markdown("---")
    st.markdown('<div class="section-title">📈 原料 使用履歴</div>', unsafe_allow_html=True)
    if material_usage_events:
        hist_mat_options = sorted(material_usage_by_name.keys())
        default_idx = hist_mat_options.index(materials[0]) if materials and materials[0] in hist_mat_options else 0
        sel_hist_mat = st.selectbox("📦 原料を選択して使用履歴を表示", hist_mat_options, index=default_idx, key="dash_hist_mat")

        mat_events = [e for e in material_usage_events if e["原料名"] == sel_hist_mat]
        df_hist = pd.DataFrame(mat_events)

        col_chart, col_stat = st.columns([3, 1])
        with col_stat:
            st.metric("累計使用量", f"{fmt_kg(df_hist['使用量(kg)'].sum())} kg")
            st.metric("使用件数", f"{len(mat_events)} 件")
            last_date = df_hist["日付"].max() if not df_hist.empty else "─"
            st.metric("直近使用日", last_date)
        with col_chart:
            if not df_hist.empty:
                df_daily = df_hist.groupby("日付", as_index=False)["使用量(kg)"].sum().sort_values("日付")
                fig = px.bar(df_daily, x="日付", y="使用量(kg)", title=f"「{sel_hist_mat}」の使用量推移(日別)")
                fig.update_layout(height=320, margin=dict(l=10, r=10, t=40, b=10), plot_bgcolor="white")
                fig.update_traces(marker_color="#ea580c")
                st.plotly_chart(fig, use_container_width=True)

        with st.expander("📋 使用履歴の詳細一覧(直近が上)"):
            df_detail = df_hist.sort_values("日付", ascending=False)[["日付", "品名", "使用量(kg)", "ロット"]]
            st.dataframe(df_detail, use_container_width=True, hide_index=True)
    else:
        st.info("使用履歴データがまだありません。製造仕込みタブで実績を登録すると、ここに使用履歴が表示されます。")
...
# ═══════════════════════════════════════════════════════════════
    ...
    if not active_recipe:
            ...
            with st.container(border=True):
                ...
                with c3:
                    if is_water:
                        ...
                        card_entries.append({"原料名": r_name, "kg": round(float(calc_kg), 2), "lot": "─"})
                        ...
                        with lot_popover("📦 ロット選択 / ブレンド設定"):
                            ...
                            if blend_on:
                                ...
                                total_kg_adj = st.number_input(
                                    "ブレンド合計投入量 (kg)", value=round(float(calc_kg), 2), step=0.01, format="%.2f",
                                ...
                                act_kg = st.number_input("実投入量微調整 (kg)", value=round(float(calc_kg), 2), step=0.01, format="%.2f", key=f"act_kg_{i}{key_suffix}_{round(calc_kg, 4)}")
                                ...
                                card_entries.append({"原料名": r_name, "kg": round(float(act_kg), 2), "lot": final_lot})
        ...
        if st.button("💾 この実績で製造記録を保存する", type="primary", use_container_width=True):
            ...
            # 【修繕】以前は集計値(k_kg等)や仕込量をそのままstr化して保存しており、
            #   浮動小数点演算の誤差(例: 150×0.014÷10 が 2.1000000000000005 のようになる等)が
            #   そのままスプレッドシートに記録され、特定の製品で小数点以下が長く
            #   表示される不具合の原因になっていた。保存直前に丸めることで解消。
            k_kg = round(k_kg, 2)
            s_kg = round(s_kg, 2)
            st_kg = round(st_kg, 2)
            ...
            sheets.append_brewing({
                ...
                "メーカー": operator, "主原料ロット": k_lot, "仕込量(kg)": round(float(target_size), 2),
                "こんにゃく精粉(kg)": k_kg, "海藻粉(kg)": s_kg, "海藻粉ロット": s_lot,
                "デンプン(kg)": st_kg, "デンプンロット": st_lot, "デンプン種別": "-",
                "石灰(kg)": lime_kg, "石灰水(L)": round(float(lime_water_size), 2),
            ...
            # ★【新規実装】製造記録の登録が視覚的にはっきりわかるよう、
            #   小さなトースト通知だけでなく、風船アニメーションと
            #   登録内容を要約した大きな確認バナーを表示するように変更。
            st.balloons()
            st.markdown(f'''
            <div style="background: linear-gradient(135deg, #16a34a, #15803d); color:white; border-radius:16px;
                        padding:26px 20px; margin-top:18px; text-align:center; box-shadow:0 10px 24px rgba(21,128,61,0.35);">
                <div style="font-size:2.6rem; line-height:1;">✅</div>
                <div style="font-size:1.5rem; font-weight:900; margin-top:6px;">製造実績を登録しました！</div>
                <div style="font-size:1.1rem; margin-top:12px; opacity:0.95; line-height:1.7;">
                    📅 {brew_date} 　 🏭 {p_name}<br>
                    仕込量 <b>{fmt_kg(target_size)}kg</b>　
                    こんにゃく精粉 <b>{fmt_kg(k_kg)}kg</b>　
                    石灰 <b>{fmt_kg(lime_kg)}kg</b>
                </div>
            </div>
            ''', unsafe_allow_html=True)
            time.sleep(2.0)
