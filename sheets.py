...
def next_arrival_no(arr):
    nums = [int(a['入荷No'].split('-')[1]) for a in arr if '入荷No' in a and '-' in str(a['入荷No'])]
    return f"A-{(max(nums + [0]) + 1):04d}"

def load_brewing():
    rows = _read("仕込み記録", COLS_BRW)
    for r in rows:
        for c in ["仕込量(kg)","こんにゃく精粉(kg)","海藻粉(kg)","デンプン(kg)","石灰(kg)","石灰水(L)"]:
            r[c] = _f(r.get(c))
...
def next_brewing_no(brw):
    return max([_i(b.get("仕込No")) for b in brw] + [0]) + 1
