...
COLS_REC = ["品名", "大カテゴリ", "中カテゴリ", "配合JSON"]
COLS_REC_LOG = ["ログID", "変更日時", "品名", "処理", "変更内容", "作業者"] # 追加: レシピ変更履歴用
...
def load_recipes():
    ...
    if not rows:
        default_recipe = [{
            "品名": "標準こんにゃく（黒）", 
            "大カテゴリ": "プラント", "中カテゴリ": "黒", 
            "配合JSON": json.dumps([{"原料名": "こんにゃく粉（国産）", "比率": 2.50}, {"原料名": "海藻粉", "比率": 0.20}, {"原料名": "石灰", "比率": 0.14}, {"原料名": "水", "比率": 97.16}], ensure_ascii=False)
        }]
        _over("配合マスタ", COLS_REC, default_recipe)
        rows = default_recipe
    return rows

def save_recipes(recs): _over("配合マスタ", COLS_REC, recs)

# --- 追加: レシピ変更履歴の読込・書込 ---
@st.cache_data(ttl=20)
def load_recipe_logs():
    return _read("レシピ変更履歴", COLS_REC_LOG)

def append_recipe_log(r):
    _append("レシピ変更履歴", COLS_REC_LOG, r)
