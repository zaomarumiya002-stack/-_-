...
# ⚠️既存データを壊さないよう、海藻粉種別は一番最後に追加しています
COLS_BRW = ["仕込No", "仕込日", "品名", "メーカー", "主原料ロット", "仕込量(kg)", "こんにゃく精粉(kg)", "海藻粉(kg)", "海藻粉ロット", "デンプン(kg)", "デンプンロット", "デンプン種別", "石灰(kg)", "石灰水(L)", "その他添加物", "備考", "登録日時", "海藻粉種別"]
...
def next_arrival_no(arr): return f"A-{(max([int(a.get('入荷No','A-0').split('-')[1]) for a in arr if '入荷No' in a and '-' in str(a.get('入荷No',''))]+[0])+1):04d}"
...
def load_order_points():
    try: return {r[0]: _f(r[1]) for r in _ws("発注点マスター", ["material", "order_point"]).get_all_values()[1:] if r and r[0]}
    except: return {}
