...
COLS_REC = ["品名", "こんにゃく精粉(kg)", "海藻粉(kg)", "デンプン(kg)", "石灰(kg)"]  # 配合比マスタ用
...
def load_recipes():
    ...
    if not rows:
        # 初期状態のデフォルトサンプルを作成
        default_recipes = [
            {"品名": "通常こんにゃく (国産)", "こんにゃく精粉(kg)": "25.0", "海藻粉(kg)": "1.0", "デンプン(kg)": "5.0", "石灰(kg)": "2.0"},
            {"品名": "白こんにゃく", "こんにゃく精粉(kg)": "24.0", "海藻粉(kg)": "0.0", "デンプン(kg)": "4.0", "石灰(kg)": "1.8"},
            {"品名": "手延べ風太麺", "こんにゃく精粉(kg)": "30.0", "海藻粉(kg)": "1.5", "デンプン(kg)": "8.0", "石灰(kg)": "2.5"}
        ]
        _over("配合マスタ", COLS_REC, default_recipes)
        return [{k: float(v) if k != "品名" else v for k, v in r.items()} for r in default_recipes]
    ...
    for r in rows:
        res.append({
            "品名": r.get("品名", ""),
            "こんにゃく精粉(kg)": _f(r.get("こんにゃく精粉(kg)")),
            "海藻粉(kg)": _f(r.get("海藻粉(kg)")),
            "デンプン(kg)": _f(r.get("デンプン(kg)")),
            "石灰(kg)": _f(r.get("石灰(kg)"))
        })
    return res

def save_recipes(recs):
    _over("配合マスタ", COLS_REC, recs)
